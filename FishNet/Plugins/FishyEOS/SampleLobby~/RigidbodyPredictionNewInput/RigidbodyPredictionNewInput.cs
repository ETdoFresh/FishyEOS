...
namespace EOSLobby.RigidbodyPredictionNewInput
{

    public class RigidbodyPredictionNewInput : NetworkBehaviour
    {
        #region Types.
        ...
        private void Update()
        {
            if (base.IsOwner)
            {
                if (Keyboard.current.spaceKey.wasPressedThisFrame && Time.time > _nextJumpTime)
                {
                    _nextJumpTime = Time.time + 1f;
                    _jump = true;
                }
                else if (Keyboard.current.leftShiftKey.wasPressedThisFrame || Mouse.current.leftButton.wasPressedThisFrame)
                {
                    _spawnBullet = true;
                }
                else if (Keyboard.current.leftAltKey.wasPressedThisFrame || Mouse.current.rightButton.wasPressedThisFrame)
                {
                    _despawnBullet = true;
                }
            }
        ...
        private void BuildMoveData(out MoveData md)
        {
            ...
            var horizontal = Keyboard.current.aKey.isPressed ? -1f : Keyboard.current.dKey.isPressed ? 1f : 0f;
            var vertical = Keyboard.current.wKey.isPressed ? 1f : Keyboard.current.sKey.isPressed ? -1f : 0f;
